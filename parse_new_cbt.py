...
def parse_args():
    ...
    parser.add_argument('-p', '--pctiles', dest='pctiles', action='store', nargs='?', const='50.00,80.00,90.00,95.00,99.00', required=False, help='Print the
specified comma-seperated latency percentiles (##.##).')
    parser.add_argument('-s', '--split', dest='split', action='store_true', default=False, required=False, help='Seperate IOPS and latency between reads and
writes.')
...
def convert_unit(unit):
    if(unit == 'B/s' or unit == 'sec'):
        ...
        return 0;
def print_header(ctx):                                                                                                                              [238/612]
    if ctx.pctiles:
        if ctx.split:
            sys.stdout.write('Benchmark, Iteration, Procs, IOSize, Pattern, Mix, IODepth, Bandwidth(KB/s), readIOPS, writeIOPS, readAvgLat(ms), writeAvgLat(m
s), readMinLat(ms), writeMinLat(ms)')
...
class Test(object):
 ...
 self.outputs.append(output)                                                                                                                 [195/612]
    ...
    def printTest(self):
        if self.ctx.pctiles:
            if self.ctx.split:
                sys.stdout.write('%s, %s, %s, %s, %s, %s, %s, %d, %d, %d, %.2f, %.2f, %.2f, %.2f' % (self.metadata['benchmark'], self.metadata['iteration'],
self.clients,
...
class Output(object):
    def __init__(self, benchmark, fn):
        ...
        self.iops = 0
        self.bw = 0
self.lat = {'avg': 0, 'min': 0, 'max': 0}                                                                                                   [109/612]
...
write_job_lat['max'].append(job['write'][lat_key]['max'])                                                                                [66/612]
...
if __name__ == '__main__':
    ...
    values = []
FORMAT = ['hash', 'testname', 'benchmark', 'iteration', 'procs', 'iosize', 'pattern', 'mix', 'iodepth', 'bandwidth', 'iops', 'avglat']           [23/612]
    ...
    for dn in ctx.DIR:
        ...
        tests.sort(key=lambda x: (x.metadata['benchmark'], x.metadata['rwmixread'], x.clients, x.metadata['op_size'], x.metadata['iteration'], x.metadata['io
depth']))
        ...
        for test in tests:
            ...
            values = (test.hashid, dn, test.metadata['benchmark'], int(test.metadata['iteration']),
                test.clients, int(test.metadata['op_size']), test.metadata['mode'], int(test.metadata['rwmixread']), int(test.metadata['iodepth']), test.bw,
test.iops, test.lat['avg'])
            c = conn.cursor()
